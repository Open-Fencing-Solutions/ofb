...
Device::BoutModeSelector* Device::BoutModeSelector::activeInstance = nullptr;

namespace Device{
  // Static interrupt handler for the switch
  void IRAM_ATTR BoutModeSelector::interruptHandler(){
    if (BoutModeSelector::activeInstance) {
      BoutModeSelector::activeInstance->interruptTriggered = true;
    }
  }

  // Constructor implementation
  BoutModeSelector::BoutModeSelector(const uint8_t selectorPins[2]){
    ...
    BoutModeSelector::activeInstance = this;
  }

  // Destructor implementation
  BoutModeSelector::~BoutModeSelector(){
    //Disable the interrupts for this switch
    this->disable();

    //Remove this instance pointer from the static class
    BoutModeSelector::activeInstance = nullptr;
  }

  void BoutModeSelector::enable(){
    ...
    attachInterrupt(digitalPinToInterrupt(this->topPin), Device::BoutModeSelector::interruptHandler, CHANGE);
    attachInterrupt(digitalPinToInterrupt(this->bottomPin), Device::BoutModeSelector::interruptHandler, CHANGE);
  ...
  Device::BoutMode BoutModeSelector::getSelectedBoutMode(){
    ...
    Device::BoutMode selectedMode = static_cast<Device::BoutMode>(switchIndex);
  ...
  void BoutModeSelector::handleInterrupts(){
    ...
    Device::BoutMode selectedMode = BoutModeSelector::getSelectedBoutMode();

    //Set the mode on the active bout
    bout.boutMode = selectedMode;
